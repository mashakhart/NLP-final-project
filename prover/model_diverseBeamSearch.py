...
class EntailmentWriter(pl.LightningModule):
    ...
    def forward(# type: ignore, 
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor, labels: torch.Tensor,) -> Any:
        return self.seq2seq(input_ids=input_ids, attention_mask=attention_mask, labels=labels).loss
    ...
    def generate_stepwise_proof(
        ...
        if not self.proof_search:
            ...
            score = [min(s) if len(s) > 0 else 0.0 for s in step_scores] # HERE!!!!
        else:
    ...
    def generate_greedy_proofs(
        ...
        for _ in range(self.max_num_steps):
            ...
            scores = [s[0] if len(s) > 0 else 0.0 for s in scores]  # HERE!!!
